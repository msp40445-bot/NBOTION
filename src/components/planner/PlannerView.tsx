...
import {
  ...
  ChevronDown, TrendingUp, Calendar, Flame, ListTodo, Repeat
} from 'lucide-react';
...
export function PlannerView() {
  const {
    goals, dailyTasks, habits, currentTimeframe, setCurrentTimeframe, addGoal,
    updateGoal, deleteGoal, addMilestone, toggleMilestone, deleteMilestone,
    getGoalsByTimeframe, addDailyTask, toggleDailyTask, deleteDailyTask,
    addHabit, toggleHabitDate, deleteHabit
  } = usePlannerStore();
  const [activeTab, setActiveTab] = useState<'goals' | 'daily' | 'habits'>('daily');
  ...
  const handleAddTask = () => {
    if (newTaskTitle.trim()) {
      addDailyTask(newTaskTitle.trim(), newTaskPriority);
      setNewTaskTitle('');
  ...
  const handleAddHabit = () => {
    if (newHabitTitle.trim()) {
      const colors = ['#10B981', '#8B5CF6', '#3B82F6', '#F59E0B', '#EC4899', '#06B6D4'];
      addHabit(newHabitTitle.trim(), 'daily', colors[habits.length % colors.length]);
      setNewHabitTitle('');
    }
  };

  const tabs = [
    { id: 'daily' as const, label: 'Daily Tasks', icon: ListTodo, count: `${completedToday}/${todayTasks.length}` },
    ...
    <div className="max-w-4xl mx-auto p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Plan your day, build habits, and track long-term goals</p>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-6">
        {tabs.map((tab) => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            <tab.icon size={16} />
            {tab.label}
            {tab.count && <span className="text-xs bg-muted px-1.5 py-0.5 rounded-full">{tab.count}</span>}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'daily' && (
        <div>
            ...
            <div className="flex items-center gap-3">
              ...
              {todayTasks.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-muted-foreground">{completedToday}/{todayTasks.length}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-card border border-border rounded-lg p-4 mb-4">
            <div className="flex gap-2">
              <input value={newTaskTitle} onChange={(e) => setNewTaskTitle(e.target.value)}
                ...
                placeholder="Add a task for today..." className="flex-1 bg-transparent border-none outline-none text-sm" />
              <select value={newTaskPriority} onChange={(e) => setNewTaskPriority(e.target.value as DailyTask['priority'])}
          ...
          <div className="space-y-2">
            {todayTasks.map((task) => (
              <div key={task.id} className={cn('flex items-center gap-3 p-3 rounded-lg border transition-colors group',
                task.completed ? 'bg-muted/30 border-border' : 'bg-card border-border hover:border-primary/30')}>
                <button onClick={() => toggleDailyTask(task.id)}>
                  {task.completed ? <CheckCircle2 size={18} className="text-green-500" /> : <Circle size={18} className="text-muted-foreground" />}
                </button>
                <span className={cn('text-sm flex-1', task.completed && 'line-through text-muted-foreground')}>{task.title}</span>
                <span className={cn('text-xs font-medium', priorityColors[task.priority])}>{task.priority}</span>
                {task.category && <span className="text-xs bg-muted px-1.5 py-0.5 rounded">{task.category}</span>}
        ...
        <div>
          <div className="bg-card border border-border rounded-lg p-4 mb-4">
            <div className="flex gap-2">
              <input value={newHabitTitle} onChange={(e) => setNewHabitTitle(e.target.value)}
                ...
                placeholder="Add a new habit..." className="flex-1 bg-transparent border-none outline-none text-sm" />
          ...
          <div className="space-y-3">
            {habits.map((habit) => {
              ...
              return (
                <div key={habit.id} className="bg-card border border-border rounded-lg p-4 group">
                  <div className="flex items-center gap-3">
                    <button onClick={() => toggleHabitDate(habit.id, today)}
                      className={cn('w-8 h-8 rounded-lg flex items-center justify-center transition-colors',
                        isDoneToday ? 'bg-green-500/20' : 'bg-muted hover:bg-muted/80')}
                      style={isDoneToday ? { backgroundColor: habit.color + '20' } : undefined}>
                      {isDoneToday ? <CheckCircle2 size={18} style={{ color: habit.color }} /> : <Circle size={18} className="text-muted-foreground" />}
                    </button>
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground">Best: {habit.bestStreak} days</span>
            ...
            {habits.length === 0 && (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-xs mt-1">Build consistent habits to achieve your goals</p>
              </div>
            )}
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        ...
                        <span className="text-xs text-muted-foreground flex items-center gap-1"><Calendar size={12} /> {goal.timeframe}</span>
